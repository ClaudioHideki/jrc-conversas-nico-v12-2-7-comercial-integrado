...
export default {
  ...
  watch: {
    $route(to, from) {
      if (
        ['settings_inbox_show', 'super_admin_settings_inbox_show'].includes(
          to.name
        )
      ) {
  ...
  methods: {
    ...
    updateRouteWithoutRefresh(selectedTabIndex) {
      ...
      const routePrefix = this.$route.path.startsWith('/super_admin/accounts')
        ? '/super_admin/accounts'
        : '/app/accounts';
      const baseUrl = `${routePrefix}/${accountId}/settings/inboxes/${inboxId}`;
