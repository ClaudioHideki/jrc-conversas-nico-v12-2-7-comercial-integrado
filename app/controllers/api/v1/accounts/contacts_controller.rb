class Api::V1::Accounts::ContactsController < Api::V1::Accounts::BaseController
  ...
  def search
    ...
    if phone_candidates.any?
      normalized_phone_scope = Current.account.contacts.where(
        "REGEXP_REPLACE(phone_number, '[^0-9]', '', 'g') IN (?)",
        phone_candidates
      )
  ...
  def normalized_phone_candidates(value)
    digits = value.gsub(/[^0-9]/, '')
    return [] unless digits.length.between?(10, 13)

    candidates = [digits]
    candidates << digits.delete_prefix('55') if digits.start_with?('55') && digits.length.between?(12, 13)
    candidates << "55#{digits}" if digits.length.between?(10, 11)
    candidates.uniq
  end
