class JrcBrokerPolicy < ApplicationPolicy
  ...
  def allowed?(action)
    ...
    delegated = JrcBrokerInboxGrant.exists?(account_id: account.id, inbox_id: record.inbox_id, user_id: user.id, can_pair: true)
    JrcBroker::Access.allowed?(administrator: account_user.reload.administrator?, assigned: assigned, delegated: delegated, action: action)
  end
end
