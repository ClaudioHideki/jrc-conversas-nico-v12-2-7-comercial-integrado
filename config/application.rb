...
module Chatwoot
  class Application < Rails::Application
    ...
    if encryption_keys_configured
      ...
      # Keep application credentials encrypted in installations that predate the
      # dedicated Active Record Encryption variables. SECRET_KEY_BASE must remain
      # stable so previously encrypted data can always be decrypted.
      derived_key = lambda do |purpose|
        OpenSSL::HMAC.hexdigest('SHA256', ENV.fetch('SECRET_KEY_BASE'), purpose)
      end

      config.active_record.encryption.primary_key = derived_key.call('active-record-encryption-primary')
      config.active_record.encryption.deterministic_key = derived_key.call('active-record-encryption-deterministic')
      config.active_record.encryption.key_derivation_salt = derived_key.call('active-record-encryption-salt')
    end

    if encryption_keys_configured || ENV['SECRET_KEY_BASE'].present?
